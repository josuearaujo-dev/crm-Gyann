...
export function PipelineBoard({
  ...
  const handleDrop = async (columnId: string) => {
    ...
    const leadRow = leads.find((l) => l.id === draggedLead);
    const patch: Record<string, unknown> = { column_id: columnId };
    if (wonColumnId && columnId === wonColumnId && leadRow && !(leadRow as any).won_at) {
        ...
        leads.map((lead) =>
          lead.id === draggedLead
            ? {
                ...lead,
                column_id: columnId,
                ...(patch.won_at ? { won_at: String(patch.won_at) } : {}),
              }
            : lead
        )
      );
