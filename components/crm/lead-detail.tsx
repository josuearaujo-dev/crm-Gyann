...
import { getLeadFaturamentoDisplay, getLeadSegmentoDisplay } from "@/lib/lead-metadata-display";
import { formatLeadDaysLabel, getLeadTimelineInfo } from "@/lib/lead-timeline";
...
export function LeadDetail({
  ...
  const faturamentoDisplay = getLeadFaturamentoDisplay(lead.metadata);
  const segmentoDisplay = getLeadSegmentoDisplay(lead.metadata);
  const leadTimeline = getLeadTimelineInfo(lead, columns);
  const leadDaysLabel = formatLeadDaysLabel(leadTimeline);
  ...
  const handleColumnChange = async (columnId: string) => {
    ...
    // Mantém histórico: só preenche won_at na primeira vez que entra na coluna de ganho
    if (wonColumnId && columnId === wonColumnId && !lead.won_at) {
  ...
  <SheetHeader className="space-y-6 p-8 pb-6 bg-muted/30">
        ...
        <div className="px-8 pb-8 space-y-8">
          <div className="grid grid-cols-1 gap-3">
            ...
            {Boolean(segmentoDisplay || lead.metadata?.segmento) && (
              <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-xl">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm text-foreground">
                    {segmentoDisplay || "Não informado"}
                  </span>
                </div>
              </div>
            )}
            {Boolean(faturamentoDisplay || lead.metadata?.faturamento) && (
              <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-xl">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm text-foreground">
                    {faturamentoDisplay || "Não informado"}
                  </span>
                </div>
              </div>
            )}
            <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-xl">
              ...
              <div className="flex flex-col gap-1">
                <div>
                  <span className="text-xs text-muted-foreground">Data de entrada</span>
                  <p className="text-sm text-foreground">
                    {formatInAppTimezone(leadTimeline.addedAt, {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                      hour: "2-digit",
                ...
                <div>
                  <span className="text-xs text-muted-foreground">
                    {leadTimeline.closedAt ? "Tempo até o fechamento" : "Tempo no funil"}
                  </span>
                  <p className="text-sm text-foreground">{leadDaysLabel}</p>
                </div>
                {leadTimeline.closedAt && (
                  <div>
                    <span className="text-xs text-muted-foreground">
                      Fechado em ({leadTimeline.statusLabel})
                    </span>
                    <p className="text-sm text-foreground">
                      {formatInAppTimezone(leadTimeline.closedAt.toISOString(), {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                        hour: "2-digit",
